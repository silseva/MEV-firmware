...
static int findActiveBlock()
{
    ...
    // Check data validity
    uint16_t crc = crc16(&(memory->data[block].params), sizeof(PidParameters));
...
bool loadParamsFromFlash(PidParameters& params)
{
    ...
    params = memory->data[block].params;
    return true;
}
...
void saveParamsToFlash(const PidParameters& params)
{
    ...
    tmpBlock.params = params;
    tmpBlock.crc    = crc16(&(tmpBlock.params), sizeof(PidParameters));
    ...
    // Save data
    addr = reinterpret_cast< uint32_t > (&(memory->data[block]));
